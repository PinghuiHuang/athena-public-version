  ...
  {using namespace HydroIntegratorTaskNames; // NOLINT (build/namespace)
    ...
    if (pm->multilevel) { // SMR or AMR
      if (NDUSTFLUIDS > 0) {
        ...
        AddTask(INT_HYD,(RECV_HYDFLX|RECV_DUSTFLUIDSFLX));
      }
      else {
        AddTask(SEND_HYDFLX,CALC_HYDFLX);
        AddTask(RECV_HYDFLX,CALC_HYDFLX);
    ...
    if (NDUSTFLUIDS > 0) {
      ...
      AddTask(SRCTERM_DUSTFLUIDS,INT_HYD);
    ...
    if (MAGNETIC_FIELDS_ENABLED) { // MHD
      ...
      if (pm->multilevel) { // SMR or AMR
        if (NDUSTFLUIDS > 0) {
          AddTask(PROLONG,(SEND_HYD|SETB_HYD|SEND_FLD|SETB_FLD|SEND_DUSTFLUIDS|SETB_DUSTFLUIDS));
        } else {
        ...
        if (SHEARING_BOX) {
          if (NDUSTFLUIDS > 0) {
            AddTask(CONS2PRIM,
            (SETB_HYD|SETB_FLD|SETB_DUSTFLUIDS|RECV_HYDSH|RECV_FLDSH|RMAP_EMFSH));
          } else {
      ...
      if (pm->multilevel) { // SMR or AMR
        if (NDUSTFLUIDS > 0) {
          AddTask(PROLONG,(SEND_HYD|SETB_HYD|SETB_DUSTFLUIDS|SEND_DUSTFLUIDS));
        } else {
        ...
        if (SHEARING_BOX) {
          if (NDUSTFLUIDS > 0) {
            AddTask(CONS2PRIM,(SETB_HYD|RECV_HYDSH|SETB_DUSTFLUIDS|RECV_DUSTFLUIDSSH));  // RECV_SCLRSH
          } else {
...
void TimeIntegratorTaskList::AddTask(const TaskID& id, const TaskID& dep) {
  ...
  //} else if (id == DRAG_DUSTGAS) {
    ...
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_DUSTFLUIDSFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::SendDustFluidsFlux);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_DUSTFLUIDSFLX) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::ReceiveDustFluidsFlux);
    task_list_[ntasks].lb_time = false;
  //} else if (id == INT_DUSTFLUIDS) {
    ...
    task_list_[ntasks].lb_time = true;
  } else if (id == SEND_DUSTFLUIDSSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&TimeIntegratorTaskList::SendDustFluidsShear);
    task_list_[ntasks].lb_time = true;
  } else if (id == RECV_DUSTFLUIDSSH) {
    task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
...
TaskStatus TimeIntegratorTaskList::IntegrateHydro(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    if (NDUSTFLUIDS > 0)
      pmb->WeightedAve(pdf->df_cons1, pdf->df_cons, pdf->df_cons2, ave_wghts);
    ...
    if (NDUSTFLUIDS > 0)
      pdf->dfdrag.Aerodynamics_Drag(pmb, wght, pdf->stopping_time_array, ph->w, pdf->df_prim, ph->u, pdf->df_cons);
    ...
    if (stage == 4 && integrator == "ssprk5_4") {
      ...
      if (NDUSTFLUIDS > 0) {
        // writing out to df_cons2 register
        pmb->WeightedAve(pdf->df_cons2, pdf->df_cons1, pdf->df_cons2, ave_wghts);
        pdf->AddDustFluidsFluxDivergence(wght_ssp, pdf->df_cons2);
        // add coordinate (geometric) source terms
        pmb->pcoord->AddCoordTermsDivergence_DustFluids(wght_ssp, pdf->df_flux, pdf->df_prim, pdf->df_cons2);
      }
    }
...
TaskStatus TimeIntegratorTaskList::PhysicalBoundary(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    // Scaled coefficient for RHS time-advance within stage
    Real dt = (stage_wghts[(stage-1)].beta)*(pmb->pmy_mesh->dt);
    // Swap Hydro and (possibly) dust fluids quantities in BoundaryVariable interface
...
TaskStatus TimeIntegratorTaskList::CalculateDustFluidsFlux(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    if ((stage == 1) && (integrator == "vl2")) {
      ...
      pdf->CalculateDustFluidsFluxes(pmb->precon->xorder, pdf->df_prim);
      return TaskStatus::next;
    }
  }
  return TaskStatus::fail;
...
TaskStatus TimeIntegratorTaskList::ReceiveDustFluidsFlux(MeshBlock *pmb, int stage) {
...
TaskStatus TimeIntegratorTaskList::SendDustFluids(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
    // Swap DustFluids quantity in BoundaryVariable interface back to conserved var
    // formulation (also needed in SetBoundariesDustFluids() since the tasks are independent)
    pmb->pdustfluids->dfbvar.var_cc = &(pmb->pdustfluids->df_cons);
...
TaskStatus TimeIntegratorTaskList::SetBoundariesDustFluids(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
    // Set DustFluids quantity in BoundaryVariable interface to cons var formulation
    pmb->pdustfluids->dfbvar.var_cc = &(pmb->pdustfluids->df_cons);
...
TaskStatus TimeIntegratorTaskList::DiffuseDustFluids(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    dfdif.CalcDustFluidsDiffusionFlux(pdf->df_prim, pdf->df_cons);
  } else {
    return TaskStatus::fail;
  }
  return TaskStatus::next;
