...
void SphericalPolar::AddCoordTermsDivergence_DustFluids(const Real dt, const AthenaArray<Real> *df_flx,
  ...
  bool do_dustfluids_diffusion = pdf->dfdif.dustfluids_diffusion_defined;
      //&& (pdf->particle_density > 0.0 || pdf->nu_dustfluids_iso > 0.0 || pdf->nu_dustfluids_aniso > 0.0));
...
#pragma omp simd
        for (int i=pmy_block->is; i<=pmy_block->ie; ++i) {
          ...
          if (do_dustfluids_diffusion && pdf->dfdif.Momentum_Diffusion_Flag_) {
