...
void OutputType::LoadOutputData(MeshBlock *pmb) {
  ...
  if (NDUSTFLUIDS > 0) {
    ...
    for (int n=0; n<4*NDUSTFLUIDS; n++) {
      num_dust     = n/4 + 1;
      num_dust_var = n%4;
      std::string dustfluids_name_cons;
      std::string dustfluids_name_prim;

      if (!(num_dust_var)){
        dustfluids_name_cons = "dust_" + std::to_string(num_dust) + "_den";
        dustfluids_name_prim = "dust_" + std::to_string(num_dust) + "_rho";
      }
      else {
        dustfluids_name_cons = "dust_" + std::to_string(num_dust) + "_mom" + std::to_string(num_dust_var);
        dustfluids_name_prim = "dust_" + std::to_string(num_dust) + "_vel" + std::to_string(num_dust_var);
      }

      if (output_params.variable.compare(dustfluids_name_cons) == 0 ||
          output_params.variable.compare("cons") == 0) {
        pod = new OutputData;
        pod->type = "SCALARS";
        pod->name = dustfluids_name_cons;
        pod->data.InitWithShallowSlice(pdfs->df_cons, 4, n, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
      if (output_params.variable.compare(dustfluids_name_prim) == 0 ||
          output_params.variable.compare("prim") == 0) {
        pod = new OutputData;
        pod->type = "SCALARS";
        pod->name = dustfluids_name_prim;
        pod->data.InitWithShallowSlice(pdfs->df_prim, 4, n, 1);
        AppendOutputDataNode(pod);
        num_vars_++;
      }
    }
