...
void RestartOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool force_write) {
  ...
  for (int b=0; b<pm->nblocal; ++b) {
    ...
    // (primitive variable) primitives of dust fluids
    // if ???
    // if (NDUSTFLUIDS > 0) {
    //   AthenaArray<Real> &prim_df = pmb->pdustfluids->df_prim;
    //   std::memcpy(pdata, prim_df.data(), prim_df.GetSizeInBytes());
    //   pdata += prim_df.GetSizeInBytes();
    // }
