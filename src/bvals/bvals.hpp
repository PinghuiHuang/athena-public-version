...
//!   (Hydro, Field, DustFluids, Gravity, etc.). Greedy allocation for worst-case
...
class BoundaryBase {
 public:
  ...
  // 1x pair (neighbor index, buffer ID) per entire SET of separate variable buffers
  // (Hydro, Field, Dust Fluids, Gravity, etc.). Greedy allocation for worst-case
  // of refined 3D; only 26 entries needed/initialized if unrefined 3D, e.g.
...
class BoundaryValues : public BoundaryBase, //public BoundaryPhysics,
 ...
 private:
  ...
  // Shearing box (shared with Field, Hydro and DustFluids)
