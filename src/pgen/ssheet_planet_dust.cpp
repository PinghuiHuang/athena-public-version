...
bool error_output;
Real tau_relax, rs, gmp, rad_planet, phi_planet, t0pot, omega_p;
      ...
      for (int i=il; i<=iu; i++) {
        ...
        if (ipert == 1) {
          ...
          if(!porb->orbital_advection_defined) {
            phydro->u(IM2, k, j, i) -= phydro->u(IDN, k, j, i)*qshear*Omega0*x1;
          }
          ...
          if (NDUSTFLUIDS > 0) {
            for (int n=0; n<NDUSTFLUIDS; ++n) {
              ...
              int v2_id   = rho_id + 2;
              int v3_id   = rho_id + 3;
              pdustfluids->df_cons(rho_id, k, j, i) = initial_D2G[n]*phydro->u(IDN, k, j, i);
              pdustfluids->df_cons(v1_id,  k, j, i) = 0.0;
              pdustfluids->df_cons(v2_id,  k, j, i) = 0.0;
              if(!porb->orbital_advection_defined) {
                pdustfluids->df_cons(v2_id, k, j, i) -= pdustfluids->df_cons(rho_id, k, j, i)*qshear*Omega0*x1;
              }
...
void MySource(MeshBlock *pmb, const Real time, const Real dt,
              ...
              AthenaArray<Real> &cons_df)
{
  potentialwell(pmb, time, dt, prim, prim_df, bcc, cons, cons_df);
  return;
}
...
void potentialwell(MeshBlock *pmb, const Real time, const Real dt,
          ...
          const AthenaArray<Real> &bcc, AthenaArray<Real> &cons, AthenaArray<Real> &cons_df)
{
  Real x_dis, y_dis, dis, sec_g, fourth_g, sixth_g, acc_x, acc_y, mom_x, mom_y, mom_dust_x, mom_dust_y;

  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
#pragma omp simd
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        x_dis = pmb->pcoord->x1v(i);
        y_dis = pmb->pcoord->x2v(j);
        dis   = sqrt(SQR(x_dis) + SQR(y_dis));

        //second order gravity
        sec_g = gmp/pow(SQR(dis)+SQR(rs),1.5);
        acc_x = sec_g*x_dis; // radial acceleration
        acc_y = sec_g*y_dis; // asimuthal acceleration

        //fourth order gravity
        //forth_g = gmp*(5*SQR(rs)+2*SQR(dis))/(2*pow(SQR(rs)+SQR(dis), 2.5));
        //acc_x   = forth_g*x_dis; // radial acceleration
        //acc_y   = forth_g*y_dis; // asimuthal acceleration
        ...
        mom_x = dt*prim(IDN, k, j, i)*acc_x;
        mom_y = dt*prim(IDN, k, j, i)*acc_y;
        cons(IM1, k, j, i) -= mom_x;
        cons(IM2, k, j, i) -= mom_y;

        if (NON_BAROTROPIC_EOS)
          cons(IEN, k, j, i) += (mom_x*prim(IM1, k, j, i) + mom_y*prim(IM2, k, j, i));
        ...
        if (NDUSTFLUIDS > 0) {
          for (int n=0; n<NDUSTFLUIDS; ++n) {
            ...
            int v3_id   = rho_id + 3;
						...
						mom_dust_x               = dt*prim_df(rho_id, k, j, i)*acc_x;
						mom_dust_y               = dt*prim_df(rho_id, k, j, i)*acc_y;
						cons_df(v1_id, k, j, i) -= mom_dust_x;
						cons_df(v2_id, k, j, i) -= mom_dust_y;
					}
        }
