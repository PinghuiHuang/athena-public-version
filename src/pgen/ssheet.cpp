  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        x1 = pcoord->x1v(i);
        x2 = pcoord->x2v(j);
        rd = d0*std::exp(-SQR(x1-0.0)/(2.0*SQR(0.15))-SQR(x2-0.0)/(2.0*SQR(0.15)))+0.1;
        rp = p0;
        if (ipert == 0) {
          ...
          for (int n=0; n<NDUSTFLUIDS; n++) {
            ...
            int v2_id   = rho_id + 2;
            int v3_id   = rho_id + 3;
            pdustfluids->df_cons(rho_id,k,j,i) = rd;
            pdustfluids->df_cons(v1_id,k,j,i) = 0.0;
            pdustfluids->df_cons(v2_id,k,j,i) -= rd*(qshear*Omega_0*x1);
