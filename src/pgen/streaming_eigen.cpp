...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  rho_gas_real  = pin->GetOrAddReal("hydro", "rho_real_gas",  0.0000224);
  rho_gas_imag  = pin->GetOrAddReal("hydro", "rho_imag_gas",  0.0000212);
  velx_gas_real = pin->GetOrAddReal("hydro", "velx_real_gas", -0.1691398);
  velx_gas_imag = pin->GetOrAddReal("hydro", "velx_imag_gas", 0.0361553);
  vely_gas_real = pin->GetOrAddReal("hydro", "vely_real_gas", 0.1336704);
  vely_gas_imag = pin->GetOrAddReal("hydro", "vely_imag_gas", 0.0591695);
  velz_gas_real = pin->GetOrAddReal("hydro", "velz_real_gas", 0.1691389);
  velz_gas_imag = pin->GetOrAddReal("hydro", "velz_imag_gas", -0.0361555);

  // Oscillation rate and Growth rate of instabilities
  omg_osi  = pin->GetOrAddReal("problem", "oscillation_rate", -0.3480127);
  omg_osi *= Omega_0;
  s_grow   = pin->GetOrAddReal("problem", "growth_rate", 0.4190204);
  s_grow  *= Omega_0;
  nwave    = pin->GetOrAddInteger("problem", "perturbation_wave", 1);
  ...
  if (NDUSTFLUIDS > 0) {
    for (int n=0; n<NDUSTFLUIDS; n++) {
      ...
      rho_dust_real[n]  = pin->GetOrAddReal("dust", "rho_real_dust_"  + std::to_string(n+1),  1.0);
      rho_dust_imag[n]  = pin->GetOrAddReal("dust", "rho_imag_dust_"  + std::to_string(n+1),  0.0);
      velx_dust_real[n] = pin->GetOrAddReal("dust", "velx_real_dust_" + std::to_string(n+1), -0.1398623);
      velx_dust_imag[n] = pin->GetOrAddReal("dust", "velx_imag_dust_" + std::to_string(n+1),  0.0372951);
      vely_dust_real[n] = pin->GetOrAddReal("dust", "vely_real_dust_" + std::to_string(n+1),  0.1305628);
      vely_dust_imag[n] = pin->GetOrAddReal("dust", "vely_imag_dust_" + std::to_string(n+1),  0.0640574);
      velz_dust_real[n] = pin->GetOrAddReal("dust", "velz_real_dust_" + std::to_string(n+1),  0.1639549);
      velz_dust_imag[n] = pin->GetOrAddReal("dust", "velz_imag_dust_" + std::to_string(n+1), -0.0233277);
    }
  }
        ...
        for (int i=is; i<=ie; i++) {
          ...
          if (NDUSTFLUIDS > 0) {
            for (int n=0; n<NDUSTFLUIDS; n++) {
              ...
              //Real delta_dust_rho  = amp*rhod0*(rho_dust_real[dust_id]*cn - rho_dust_imag[dust_id]*sn);
              ...
              //Real delta_gas_rho  = amp*rhod0*(ran2(&iseed) - 0.5);
              //Real delta_gas_vel1 = amp*etaVk*iso_cs*(ran2(&iseed) - 0.5);
              //Real delta_gas_vel2 = amp*etaVk*iso_cs*(ran2(&iseed) - 0.5);
              //Real delta_gas_vel3 = amp*etaVk*iso_cs*(ran2(&iseed) - 0.5);
    ...
    for (int k=ks; k<=ke; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie; i++) {
          ...
          Real delta_gas_rho  = amp*rhog0*PertEven(rho_gas_real,         rho_gas_imag,  x_dis, z_dis, 0);
          Real delta_gas_vel1 = amp*etaVk*iso_cs*PertEven(velx_gas_real, velx_gas_imag, x_dis, z_dis, 0);
          Real delta_gas_vel2 = amp*etaVk*iso_cs*PertOdd(velz_gas_real,  velz_gas_imag, x_dis, z_dis, 0);
          Real delta_gas_vel3 = amp*etaVk*iso_cs*PertEven(vely_gas_real, vely_gas_imag, x_dis, z_dis, 0);
          ...
          if (NDUSTFLUIDS > 0) {
            for (int n=0; n<NDUSTFLUIDS; n++) {
              ...
              Real delta_dust_rho  = amp*rhod0*PertEven(rho_dust_real[dust_id], rho_dust_imag[dust_id], x_dis, z_dis, 0);
              Real delta_dust_vel1 = amp*etaVk*iso_cs*PertEven(velx_dust_real[dust_id], velx_dust_imag[dust_id], x_dis, z_dis, 0);
              Real delta_dust_vel2 = amp*etaVk*iso_cs*PertOdd(velz_dust_real[dust_id],  velz_dust_imag[dust_id], x_dis, z_dis, 0);
              Real delta_dust_vel3 = amp*etaVk*iso_cs*PertEven(vely_dust_real[dust_id], vely_dust_imag[dust_id], x_dis, z_dis, 0);
