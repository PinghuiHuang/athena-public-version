...
OrbitalAdvection::OrbitalAdvection(MeshBlock *pmb, ParameterInput *pin)
    : pmb_(pmb), pm_(pmb->pmy_mesh), ph_(pmb->phydro),
      pf_(pmb->pfield), pco_(pmb->pcoord), pbval_(pmb->pbval), pdf_(pmb->pdustfluids) {
  ...
  if (pmb->pmy_mesh->OrbitalVelocity_ == nullptr) {
    ...
    if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
      OrbitalVelocity              = CartOrbitalVelocity;   // -q*Omega*x
      OrbitalVelocityDerivative[0] = CartOrbitalVelocity_x; // -q*Omega
      OrbitalVelocityDerivative[1] = ZeroOrbitalVelocity;   // 0.0
    } else if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
      if (pmb->block_size.nx3>1) { // 3D
        OrbitalVelocity              = CylOrbitalVelocity3D;   // vk = sqrt(GM/d) * x/d - Omega*x, d = sqrt(x^2+z^2)
        OrbitalVelocityDerivative[0] = CylOrbitalVelocity3D_r; // d(vk)/dx
        OrbitalVelocityDerivative[1] = CylOrbitalVelocity3D_z; // d(vk)/dz
      } else { // 2D
        OrbitalVelocity              = CylOrbitalVelocity2D;   // vk = sqrt(GM/x) - Omega*x
        OrbitalVelocityDerivative[0] = CylOrbitalVelocity2D_r; // d(vk)/dx
        OrbitalVelocityDerivative[1] = ZeroOrbitalVelocity;    // 0.0
      }
    } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
      OrbitalVelocity              = SphOrbitalVelocity;      // vk = sqrt(GM/(r*sin(th))) - Omega*r*sin(th)
      OrbitalVelocityDerivative[0] = SphOrbitalVelocity_r;    // d(vk)/dr
      OrbitalVelocityDerivative[1] = SphOrbitalVelocity_t;    // d(vk)/d(th)
    }
